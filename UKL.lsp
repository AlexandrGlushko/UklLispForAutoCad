...
(defun Enter_Point()
	...
	(setq Elev (- (caddr P1) (caddr P2)))		;Первышение P1 - P2 = Elev
...
;DrawAngle - угло для отрисовки знака уклона
...
;;;===================================================================================================
;;; Имена стелй из стандартного набора по запросу
...
(defun Calc_Grad(gType /  tmp)
	(cond
		...
		((= gType 3) (setq Grad (strcat (rtos (abs (* (/ Elev Dist) 100))2 0)"%")))		;проценты %
		((= gType 4) (setq Grad (strcat (rtos (abs (* (/ Elev Dist) 1000))2 0)"‰")))	;промили ‰
		((= gType 5)
